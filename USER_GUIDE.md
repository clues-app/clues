...
- **Replay** (the **▶ selects**) - pick a field (usually the date), and the dots **appear one by one in that order, accumulating** into the full picture, with a **ticker** showing the current value; the loop restarts after a beat and runs until you press **⏸**. Speed: Normal / Slow / 0.5x (extra slow, for long time series) / Fast; loops: x1 / x2 (default) / x3 / x5 / infinite - after the last loop the map rests at the full picture. An optional **scrubber slider** overlays the bottom of the map - drag to any point of the replay, playing or not (dragging pauses the loop). The play state **saves with the session**, so a shared deck can open animating - earthquakes spreading across the map as the dates tick by. (AI: `chart_display` with `replay_field`, `replay`, `replay_speed`.)
...
- **Replay** (the **. selects**) - like the points map: pick an order field and the regions **recolor step by step** with the running per-region average as the ticker advances, then loop. **Frame content** is your choice: the **running average** of everything so far (right for event data), or the **latest value** per region (right for time series - monthly rates and the like, where each frame is that period's map). The order is usually a **date** - but it can be the **metric itself**: with a single-period dataset (say, production by state for one year), replay by the metric lights the regions up **from lowest to highest**. Speed and loop settings match the points map.
- **Scale min / max / mid** - pin the color scale (e.g. 2 and 15 for unemployment rates) so colors mean the same thing across filters and replay frames; values beyond the bounds **saturate**, which is what makes a true anomaly read as one. **mid** centers the scale on a norm - pair it with a diverging palette (RdBu) to color regions by deviation. Empty = automatic.
...
- **Images (deck assets)** - link a folder of images (or add files) and Clues embeds **resized copies inside the session**, so decks stay self-contained and images travel with every shared `.clue`. Descriptions come from filenames or an optional `captions.json`, and a running size meter warns before the session gets heavy. Re-adding a file with the **same name replaces it**; a **Remove N unused** button sweeps every image nothing points at (the usual leftovers after re-adding images under new names), and already-optimized files keep their original bytes instead of being re-encoded larger. Place one from any chart's **⋮ menu → Image beside Chart** (left / right / above / below), then drag it freely ([§9](#9-per-chart-controls-notes--snapshots)); the AI sees the library and places images itself (`deck_image`).
